#!/usr/bin/env python3
"""Test the evidence bundle + skill-dispatch protocol for ndt-engineer
(PROTOCOL.md v1).

Verifies:
- build --bundle emits evidence/{model,gates,provenance}.json
- bundle JSON parses; model contains the computed method-selection rows
  and parameter cards; gates all_pass; exit code 0
- skill dispatch cross-checks core numbers against the bound ndt leaf
...
- build --leak-limit-sccs re-runs the disposition
...
EXPECTED_DISPATCH_FNS = [
    "select_frequency_for_flaw",      # eddy-current-inspection
    "capillary_pressure",             # liquid-penetrant-inspection
    "exposure_time",                  # radiographic-inspection
    "projection_count",               # computed-tomography
    "pressure_decay_rate",            # leak-testing
]
...
class TestNdtCliBundle(unittest.TestCase):
    ...
    def test_model_content_and_gates(self):
        ...
        m = self._model()
        self.assertEqual(m["role"], "ndt-engineer")
        self.assertEqual(m["status"], "draft-for-review")
        self.assertEqual(
            m["document_type"],
            "Nondestructive Test Plan and Method Selection Report")
        self.assertEqual([x["top_method"] for x in m["defect_population"]],
                         ["ET", "UT", "UT"])
        self.assertIn("eddy-current-surface", m["parameter_cards"])
        self.assertIn("liquid-penetrant", m["parameter_cards"])
        self.assertIn("radiography", m["parameter_cards"])
        self.assertIn("computed-tomography", m["parameter_cards"])
        self.assertIn("leak-test", m["parameter_cards"])
        g = self._gates()
    ...
    def test_standalone_honest_bundle(self):
        ...
        m = self._model()
        self.assertTrue(m["defect_population"])
        g = self._gates()
        self.assertTrue(g["all_pass"])

    def test_leak_limit_override_changes_disposition(self):
        r = run_build(self.tmp, ["--leak-limit-sccs", "0.1"])
        self.assertEqual(r.returncode, 0)
        m = self._model()
        self.assertEqual(m["parameter_cards"]["leak-test"]["verdict"],
                         "reject")
        # plan gates remain green; the disposition data is honest
    ...
    def test_check_command_gate_vocabulary(self):
        ...
        self.assertIn("RESULT: PASS", c.stdout)
        for name in ("has_title", "has_selection", "has_personnel",
                     "has_not_approval"):
