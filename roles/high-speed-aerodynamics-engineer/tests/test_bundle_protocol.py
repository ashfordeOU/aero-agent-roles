...
- skill dispatch cross-checks the core against bound AeroSkills leaf
  logic (isentropic ratios, normal shock, oblique shock, boundary
  layer) when AeroSkills is present
...
class TestBundleProtocol(unittest.TestCase):
    ...
    def test_model_has_numbers_and_status(self):
        ...
        self.assertIn("area_ratio", m)
        self.assertIn("shocks", m)
        self.assertIn("recovery_total", m["shocks"])
    ...
    def test_gates_all_pass_exit_zero(self):
        ...
        self.assertGreaterEqual(len(g["gates"]), 4)
    ...
    def test_dispatch_crosschecks_when_skills_present(self):
        ...
        self.assertGreaterEqual(len(p["skills"]), 3)
        for row in p["skills"]:
            self.assertTrue(row["dispatched"])
            self.assertTrue(row["agrees"],
                            "core/skill disagree: %s" % row)
            self.assertLessEqual(row["delta"], 1e-4)
